...
| **Text Encoder** | `t5xxl_fp8_e4m3fn.safetensors` | SHA256: `7d330da4816c802874de3ee75db0c6bf23d149021966aab7bb48e3d0faec68e7` |
...
| **ComfyUI Backend** | `v0.33.1` (Commit: `8aef9f3`) | Submodule pinned at `comfyui/` |
...
2. Start or detect the local ComfyUI backend on `127.0.0.1:8188`.
...
| **Peak VRAM Usage** | ~3.8 GB / 8.0 GB (47.5%) | Zero OOM |
| **Peak RAM Usage** | ~7.2 GB / 16.0 GB (45.0%) | Safe system headroom |

---

## 4. Privacy & Offline Invariant

- **Zero Outbound Telemetry**: All network operations are strictly bound to `127.0.0.1` (localhost).
- **Offline Capable**: Once model weights and standalone Vulkan tools are present, zero internet access is required.
...
3. **Native Generation Resolution**: Base diffusion generates at `512x288` to guarantee under ~60s generation time and zero OOM on 8GB VRAM. High-definition output is achieved via the integrated 2x Vulkan upscaler.
4. **Hardware Compatibility**: The Pascal GTX 1070 path relies on the pinned `torch==2.13.0+cu126` and ComfyUI `--lowvram --fp8_e4m3fn-text-enc` flags.
